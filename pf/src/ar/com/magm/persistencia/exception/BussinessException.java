package ar.com.magm.persistencia.exception;
...
public class BussinessException extends Exception {
	...
	private String getCaptions(Class clazz, Path path) {
		...
		if (path != null) {
			...
			for (Path.Node node : path) {
				ClassAndCaption clazzAndCaption = getSingleCaption(
						currentClazz, node.getName());
	...
	private ClassAndCaption getSingleCaption(Class clazz, String fieldName) {
		...
		if ((clazzAndCaptionField != null)
				&& (clazzAndCaptionField.caption != null)) {
			return clazzAndCaptionField;
		}

		clazzAndCaptionMethod = getMethodCaption(clazz, fieldName);
		if ((clazzAndCaptionMethod != null)
				&& (clazzAndCaptionMethod.caption != null)) {
	...
	private ClassAndCaption getMethodCaption(Class clazz, String methodName) {
		...
		Method method = ReflectionUtils.findMethod(clazz, "get"
				+ suffixMethodName);
		if (method == null) {
